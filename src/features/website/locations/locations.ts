      ...
      >
        @for (location of store.locations(); track location._id) {
          <div style="border:1px solid #EEF0FA;border-radius:18px;padding:26px;">
            ...
            <ul
              style="list-style:none;padding:0;margin:0 0 16px;display:flex;flex-direction:column;gap:8px;font-size:13px;color:#3A3D5C;"
            >
            ...
            <div style="font-size:12px;color:#8386AC;margin-bottom:16px;">
              {{ location.description }}
            </div>
            <a
              [routerLink]="routes.VpsHosting"
              style="display:block;text-align:center;padding:11px;border-radius:10px;background:linear-gradient(135deg,#1269E8,#7C3AED);color:#fff;font-weight:700;font-size:13.5px;"
              >Deploy in {{ location.city }} &rarr;</a
            >
          </div>
        } @empty {
          <p
            style="grid-column:1/-1;text-align:center;color:#8386AC;font-size:14px;padding:28px 0;"
          >
